          ...
          >
            <b-card-title>
              <h1 class="name">
                <b-link
                  class="link"
                  :href="details.restaurant.url"
                  target="_blank"
                >{{details.restaurant.name}}</b-link>
              </h1>
            </b-card-title>
            <hr />
            <b-card-text>
            ...
            <template v-slot:footer>You make miso happy!</template>
          </b-card>
        </div>
      </div>
    </b-col>
...
export default {
    ...
    choices: function() {
      ...
      fetch(
        `https://developers.zomato.com/api/v2.1/search?start=${
          this.startParam
        }&lat=${this.details.latitude}&lon=${
          this.details.longitude
        }&radius=&radius=10000&cuisines=${this.choices.toString()}`,
        {
          method: "GET",
          headers: {
  ...
  methods: {
    ...
    likeRestaurant() {
      ...
      for (
        let i = 0;
        i < this.currentRestaurant[0].restaurant.price_range;
        i++
      ) {
        dollarSymbols += "$";
      }
      ...
      let payload = {
        ...
        rating: this.currentRestaurant[0].restaurant.user_rating
          .aggregate_rating
      };
